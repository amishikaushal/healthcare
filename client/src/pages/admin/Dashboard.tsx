  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map(s => (
          <div key={s.label} className="glass-hover p-5 rounded-2xl">
            ...
            <div className="text-2xl font-bold text-surface-900">{s.value.toLocaleString()}</div>
      ...
      <div className="glass p-5 rounded-2xl">
        ...
        <div className="space-y-3">
          {recentActivity.map((a, i) => (
            <div key={i} className="flex items-center gap-3 py-2 border-b border-surface-200 last:border-0">
              <div className="w-7 h-7 rounded-xl bg-surface-100 flex items-center justify-center text-xs font-bold text-surface-500 shrink-0">
                {a.user[0]}
              </div>
              <div className="flex-1">
                <span className="text-sm text-surface-700 font-medium">{a.user}</span>
                <span className="text-sm text-surface-500"> · {a.action}</span>
              </div>
      ...
      <div className="glass p-5 rounded-2xl flex items-center gap-4">
        ...
        <div>
          <h3 className="text-sm font-semibold text-surface-900">System Configuration</h3>
          <p className="text-xs text-surface-500 mt-0.5">Manage roles, permissions, email templates and integrations</p>
        </div>
